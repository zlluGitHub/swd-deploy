...
//文件夹目录
let distDir = "", distZipPath = "", projectName = "";
...
const cloneProject = async () => {
    await SSH.exec(`git clone -b ${config.git.branch} ${config.git.url}`, [], {
        cwd: config.wwwPath
    }).catch(async err => {
        errorLog(err)
        process.exit() //退出流程
...
const pullProject = async () => {
    await SSH.exec(`git pull`, [], {
        cwd: `${config.wwwPath}/${projectName}`
    }).catch(async err => {
        errorLog(err)
        process.exit() //退出流程
    })
...
const buildProject = async () => {
    let cwd = config.wwwPath
    if (config.build.path && (config.build.path.indexOf(cwd) > -1)) {
        cwd = config.build.path
    } else if (config.build.path === '/') {
        cwd = `${config.wwwPath}/${projectName}`
    } else {
        cwd = `${config.wwwPath}/${projectName}/${config.build.path}`
    }
    await SSH.exec(config.build.run, [], { cwd }).catch(async err => {
        errorLog(err)
        process.exit() //退出流程
    })
}
...
const runProject = async () => {
    let cwd = config.wwwPath
    if (config.start.path && (config.start.path.indexOf(cwd) > -1)) {
        cwd = config.start.path
    } else if (config.start.path === '/') {
        cwd = `${config.wwwPath}/${projectName}`
    } else {
        cwd = `${config.wwwPath}/${projectName}/${config.start.path}`
    }
    await SSH.exec(config.start.run, [], { cwd }).catch(async err => {
        errorLog(err)
        process.exit() //退出流程
    })
}
...
const stopProject = async () => {
    ...
    await SSH.exec(config.start.stop, [], { cwd }).catch(async err => {
        errorLog(err)
        process.exit() //退出流程
    })
}
...
const execFlowWork = (env) => {
    ...
    if (config.git.url) {
        let startIndex = config.git.url.lastIndexOf('/');
        let endIndex = config.git.url.lastIndexOf('.');
        projectName = config.git.url.slice(startIndex + 1, endIndex);
    }
...
//         }])
//         .then(answers => {
//             execFlowWork(answers.env)
//         })
// } else if (CONFIG.development) {
//     execFlowWork('development')
// } else if (CONFIG.production) {
//     execFlowWork('production')
// } else {
